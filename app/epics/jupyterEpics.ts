...
import { LAUNCH_KERNEL, REQUEST_KERNEL_INFO, LOAD_EPOCHS, LOAD_CLEANED_EPOCHS, LOAD_PSD, LOAD_ERP, LOAD_TOPO, CLEAN_EPOCHS, CLOSE_KERNEL, loadTopo, loadERP } from "../actions/jupyterActions";
import { imports, utils, loadCSV, loadCleanedEpochs, filterIIR, epochEvents, requestEpochsInfo, requestChannelInfo, cleanEpochsPlot, plotPSD, plotERP, plotTopoMap, saveEpochs } from "../utils/jupyter/cells";
import { EMOTIV_CHANNELS, EVENTS, DEVICES, MUSE_CHANNELS, JUPYTER_VARIABLE_NAMES } from "../constants/constants";
import { parseSingleQuoteJSON, parseKernelStatus, debugParseMessage } from "../utils/jupyter/functions";
...
const launchEpic = action$ => action$.ofType(LAUNCH_KERNEL).pipe(mergeMap(() => from(find('brainwaves'))), tap(kernelInfo => {
  if (isNil(kernelInfo)) {
    toast.error("Could not find 'brainwaves' jupyter kernel. Have you installed Python?");
  }
}), filter(kernelInfo => !isNil(kernelInfo)), mergeMap(kernelInfo => from(launchSpec(kernelInfo.spec, {
...
const setUpChannelEpic = action$ => action$.ofType(SET_KERNEL).pipe(pluck('payload'), mergeMap(kernel => from(createMainChannel(kernel.config))), tap(mainChannel => mainChannel.next(executeRequest(imports()))), tap(mainChannel => mainChannel.next(executeRequest(utils()))), map(setMainChannel));

const receiveChannelMessageEpic = (action$, state$) => action$.ofType(SET_MAIN_CHANNEL).pipe(mergeMap(() => state$.value.jupyter.mainChannel.pipe(map(msg => {
...
const requestKernelInfoEpic = (action$, state$) => action$.ofType(REQUEST_KERNEL_INFO).pipe(filter(() => state$.value.jupyter.mainChannel), map(() => state$.value.jupyter.mainChannel.next(kernelInfoRequest())), ignoreElements());
...
const loadEpochsEpic = (action$, state$) => action$.ofType(LOAD_EPOCHS).pipe(pluck('payload'), filter(filePathsArray => filePathsArray.length >= 1), map(filePathsArray => state$.value.jupyter.mainChannel.next(executeRequest(loadCSV(filePathsArray)))), awaitOkMessage(action$), execute(filterIIR(1, 30), state$), awaitOkMessage(action$), map(() => epochEvents({
  ...
  console.log('e', e);
}), map(epochEventsCommand => state$.value.jupyter.mainChannel.next(executeRequest(epochEventsCommand))), awaitOkMessage(action$), map(() => getEpochsInfo(JUPYTER_VARIABLE_NAMES.RAW_EPOCHS)));

const loadCleanedEpochsEpic = (action$, state$) => action$.ofType(LOAD_CLEANED_EPOCHS).pipe(pluck('payload'), filter(filePathsArray => filePathsArray.length >= 1), map(filePathsArray => state$.value.jupyter.mainChannel.next(executeRequest(loadCleanedEpochs(filePathsArray)))), awaitOkMessage(action$), mergeMap(() => of(getEpochsInfo(JUPYTER_VARIABLE_NAMES.CLEAN_EPOCHS), getChannelInfo(), loadTopo())));

const cleanEpochsEpic = (action$, state$) => action$.ofType(CLEAN_EPOCHS).pipe(execute(cleanEpochsPlot(), state$), mergeMap(() => action$.ofType(RECEIVE_STREAM).pipe(pluck('payload'), filter(msg => msg.channel === 'iopub' && msg.content.text.includes('Channels marked as bad')), take(1))), map(() => state$.value.jupyter.mainChannel.next(executeRequest(saveEpochs(getWorkspaceDir(state$.value.experiment.title), state$.value.experiment.subject)))), awaitOkMessage(action$), map(() => getEpochsInfo(JUPYTER_VARIABLE_NAMES.RAW_EPOCHS)));

const getEpochsInfoEpic = (action$, state$) => action$.ofType(GET_EPOCHS_INFO).pipe(pluck('payload'), map(variableName => state$.value.jupyter.mainChannel.next(executeRequest(requestEpochsInfo(variableName)))), mergeMap(() => action$.ofType(RECEIVE_EXECUTE_RESULT).pipe(pluck('payload'), filter(msg => msg.channel === 'iopub' && !isNil(msg.content.data)), pluck('content', 'data', 'text/plain'), filter(msg => msg.includes('Drop Percentage')), take(1))), map(epochInfoString => parseSingleQuoteJSON(epochInfoString).map(infoObj => ({
  name: Object.keys(infoObj)[0],
  value: infoObj[Object.keys(infoObj)[0]]
}))), map(setEpochInfo));

const getChannelInfoEpic = (action$, state$) => action$.ofType(GET_CHANNEL_INFO).pipe(execute(requestChannelInfo(), state$), mergeMap(() => action$.ofType(RECEIVE_EXECUTE_RESULT).pipe(pluck('payload'), filter(msg => msg.channel === 'iopub' && !isNil(msg.content.data)), pluck('content', 'data', 'text/plain'), // Filter to prevent this from reading requestEpochsInfo returns
filter(msg => !msg.includes('Drop Percentage')), take(1))), map(channelInfoString => setChannelInfo(parseSingleQuoteJSON(channelInfoString))));

const loadPSDEpic = (action$, state$) => action$.ofType(LOAD_PSD).pipe(execute(plotPSD(), state$), mergeMap(() => action$.ofType(RECEIVE_DISPLAY_DATA).pipe(pluck('payload'), // PSD graphs should have two axes
filter(msg => msg.content.data['text/plain'].includes('2 Axes')), pluck('content', 'data'), take(1))), map(setPSDPlot));
...
const loadERPEpic = (action$, state$) => action$.ofType(LOAD_ERP).pipe(pluck('payload'), map(channelName => {
...
}), map(channelIndex => state$.value.jupyter.mainChannel.next(executeRequest(plotERP(channelIndex)))), mergeMap(() => action$.ofType(RECEIVE_DISPLAY_DATA).pipe(pluck('payload'), // ERP graphs should have 1 axis according to MNE
filter(msg => msg.content.data['text/plain'].includes('1 Axes')), pluck('content', 'data'), take(1))), map(setERPPlot));

const closeKernelEpic = (action$, state$) => action$.ofType(CLOSE_KERNEL).pipe(map(() => {
  state$.value.jupyter.kernel.spawn.kill();
  state$.value.jupyter.mainChannel.complete();
}), ignoreElements());

export default combineEpics(launchEpic, setUpChannelEpic, requestKernelInfoEpic, receiveChannelMessageEpic, loadEpochsEpic, loadCleanedEpochsEpic, cleanEpochsEpic, getEpochsInfoEpic, getChannelInfoEpic, loadPSDEpic, loadTopoEpic, loadERPEpic, closeKernelEpic);