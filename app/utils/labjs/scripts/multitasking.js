...
let tasksParameters = []
const blocks = this.parameters.block === 'mixed' ? ['shape', 'filling'] : [this.parameters.block, this.parameters.block]
...
for (let i = 1; i <= numberBlocks; i++){
  for (let block of blocks){
    if (block === 'shape'){
      tasksParameters = tasksParameters.concat(trialConstructor(block, 2, 'diamond', 'b'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 2, 'square', 'n'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 3, 'diamond', 'b'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 3, 'square', 'n'))
    } else if(block === 'filling'){
      tasksParameters = tasksParameters.concat(trialConstructor(block, 2, 'diamond', 'b'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 2, 'square', 'b'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 3, 'diamond', 'n'))
      tasksParameters = tasksParameters.concat(trialConstructor(block, 3, 'square', 'n'))
    }
  }
}
...
this.options.templateParameters = tasksParametersShuffled.slice(0, this.parameters.num_trials)
}
                  },
                  "title": "Trial loop",
                  "shuffleGroups": [],
