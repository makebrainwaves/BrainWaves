...
export const storeExperimentState = (state: ExperimentStateType) => {
  ...
  fs.writeFileSync(path.join(getWorkspaceDir(timestampedState.title), 'appState.json'), JSON.stringify(timestampedState));
};
...
export const storeBehaviouralData = (csv: string, title: string, subject: string, group: string, session: number) => {
...
export const readWorkspaceRawEEGData = async (title: string) => {
  try {
    ...
    const rawFiles = files.filter(filepath => filepath.slice(-7).includes('raw.csv')).map(filepath => ({
...
export const readWorkspaceCleanedEEGData = async (title: string) => {
  try {
    const files = await recursive(getWorkspaceDir(title));
    return files.filter(filepath => filepath.slice(-7).includes('epo.fif')).map(filepath => ({
      name: path.basename(filepath),
      path: filepath
    }));
  } catch (e) {
...
export const readWorkspaceBehaviorData = async (title: string) => {
  try {
    const files = await recursive(getWorkspaceDir(title));
    const behaviorFiles = files.filter(filepath => filepath.slice(-12).includes('behavior.csv')).map(filepath => ({
      name: path.basename(filepath),
      path: filepath
    }));
...
export const readImages = (dir: string) => fs.readdirSync(dir).filter(filename => {
  const extension = filename.slice(-3).toLowerCase();
  return (extension === 'png' || extension === 'jpg' || extension === 'gif' || extension === 'peg' // support .jpeg?
  );
});

// Returns an array of images that are used in a timeline for use in preloading
export const getImages = (params: ExperimentParameters) => readdirSync(params.stimulus1.dir).map(filename => path.join(params.stimulus1.dir, filename)).concat(readdirSync(params.stimulus2.dir).map(filename => path.join(params.stimulus2.dir, filename)));

// -----------------------------------------------------------------------------------------------
// Util

export const getSubjectNamesFromFiles = (filePaths: Array<string | null | undefined>) => filePaths.map(filePath => path.basename(filePath)).map(fileName => fileName.substring(0, fileName.indexOf('-')));
