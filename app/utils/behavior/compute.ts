...
export const aggregateBehaviorDataToSave = (data, removeOutliers) => {
  ...
  const aggregatedData = processedData.map(e => {
    ...
    for (const condition of conditions) {
      const rt = e.filter(row => row.response_given === 'yes').filter(row => row.correct_response === 'true').filter(row => row.condition === condition).map(row => row.reaction_time).map(value => parseFloat(value));
      rtMean[condition] = Math.round(ss.mean(rt));
      const accuracy = e.filter(row => row.condition === condition && row.correct_response === 'true' && row.response_given === 'yes');
...
const filterData = (data, removeOutliers) => {
  let filteredData = data.data.filter(row => row.trial_number && row.phase !== 'practice').map(row => ({
  ...
  switch (displayMode) {
    ...
    case 'errorbars':
      ...
      dataToPlot = conditions.reduce((obj, condition) => {
        ...
        const y_bars_prep = x.map(a => data_condition.map(d => d.filter(e => e.subject === a)).filter(d => d.length > 0));
  ...
  switch (displayMode) {
    ...
    case 'errorbars':
      dataToPlot = conditions.reduce((obj, condition, i) => {
        ...
        const y = subjects.map(subject => ss.mean(transformedData.filter(e => e.subject === subject).map(d => d.accuracy)));
        const stErrorFunction = array => ss.sampleStandardDeviation(array) / Math.sqrt(array.length);
