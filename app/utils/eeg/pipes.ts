...
export const parseMuseSignalQuality = () => pipe(map(epoch => ({
  ...epoch,
  signalQuality: Object.assign({}, ...Object.entries(epoch.signalQuality).map(([channelName, signalQuality]) => {
...
export const parseEmotivSignalQuality = () => pipe(map(epoch => ({
  ...epoch,
  signalQuality: Object.assign({}, ...Object.entries(epoch.signalQuality).map(([channelName, signalQuality]) => {
    if (signalQuality === 0) {
      return { [channelName]: SIGNAL_QUALITY.DISCONNECTED };
