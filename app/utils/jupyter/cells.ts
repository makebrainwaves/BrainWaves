...
export const loadCSV = (filePathArray: Array<string>) => [`files = [${filePathArray.map(filePath => formatFilePath(filePath))}]`, `replace_ch_names = None`, `raw = load_data(files, replace_ch_names)`].join('\n');

export const loadCleanedEpochs = (filePathArray: Array<string>) => [`files = [${filePathArray.map(filePath => formatFilePath(filePath))}]`, `clean_epochs = concatenate_epochs([read_epochs(file) for file in files])`, `conditions = OrderedDict({key: [value] for (key, value) in clean_epochs.event_id.items()})`].join('\n');

// NOTE: this command includes a ';' to prevent returning data
export const filterIIR = (lowCutoff: number, highCutoff: number) => `raw.filter(${lowCutoff}, ${highCutoff}, method='iir');`;

export const plotPSD = () => [`%matplotlib inline`, `raw.plot_psd(fmin=1, fmax=30)`].join('\n');

export const epochEvents = (eventIDs: {
  [key: string]: number;
}, tmin: number, tmax: number, reject?: Array<string> | string = 'None') => {
  const IDs = Object.keys(eventIDs).filter(k => k !== '').reduce((res, key) => (res[key] = eventIDs[key], res), {});
  const command = [`event_id = ${JSON.stringify(IDs)}`, `tmin=${tmin}`, `tmax=${tmax}`, `baseline= (tmin, tmax)`, `picks = None`, `reject = ${reject}`, 'events = find_events(raw)', `raw_epochs = Epochs(raw, events=events, event_id=event_id,
...
export const plotERP = (channelIndex: number) => [`%matplotlib inline`, `X, y = plot_conditions(clean_epochs, ch_ind=${channelIndex}, conditions=conditions,
    ci=97.5, n_boot=1000, title='', diff_waveform=None)`].join('\n');
