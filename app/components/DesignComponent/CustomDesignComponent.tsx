...
export default class CustomDesign extends Component<Props, State> {
  ...
  endPreview() {// this.setState({ isPreviewing: false });
  }
  ...
  renderSectionContent() {
    const stimi = [{ name: 'stimulus1', number: 1 }, { name: 'stimulus2', number: 2 }, { name: 'stimulus3', number: 3 }, { name: 'stimulus4', number: 4 }];
    switch (this.state.activeStep) {
      case CUSTOM_STEPS.OVERVIEW:default:
        return <Grid stretched relaxed padded columns='equal' className={styles.contentGrid}>
            <Grid.Column stretched verticalAlign='middle'>
              <Image as={Segment} basic centered src={researchQuestionImage} className={styles.overviewImage} />
              <Form>
                <Form.TextArea autoHeight style={{ minHeight: 100, maxHeight: 400 }} label={FIELDS.QUESTION} value={this.state.description.question} placeholder='Explain your research question here.' onChange={(event, data) => this.setState({
                description: {
            ...
            <Grid.Column stretched verticalAlign='middle'>
              ...
              <Form>
                <Form.TextArea autoHeight style={{ minHeight: 100, maxHeight: 400 }} label={FIELDS.HYPOTHESIS} value={this.state.description.hypothesis} placeholder='Describe your hypothesis here.' onChange={(event, data) => this.setState({
                description: {
                  ...this.state.description,
            ...
            <Grid.Column verticalAlign='middle'>
              ...
              <Form>
                <Form.TextArea autoHeight style={{ minHeight: 100, maxHeight: 400 }} label={FIELDS.METHODS} value={this.state.description.methods} placeholder='Explain how you will design your experiment to answer the question here.' onChange={(event, data) => this.setState({
                description: {
                  ...this.state.description,
                  methods: data.value
                },
        ...
        return <Grid>
            ...
            <Table basic='very'>
              ...
              <Table.Body className={styles.experimentTable}>
                ...
                name,
                number
              }) => <StimuliDesignColumn key={number} num={number} {...this.state.params[name]} numberImages={this.state.params.stimuli.filter(trial => trial.type === number).length} onChange={async (key, data, changedName) => {
                await this.setState({
                  params: {
                    ...this.state.params,
        ...
        return <Grid>
              ...
              <div>
                <Form style={{ alignSelf: 'flex-end' }}>
                  <Form.Group className={styles.trialsTopInfoBar}>
                    <Form.Select fluid selection label='Order' value={this.state.params.randomize} onChange={(event, data) => this.setState({
                    params: {
                      ...this.state.params,
                      randomize: data.value
                    },
                    saved: false
                  })} placeholder='Response' options={[{ key: 'random', text: 'Random', value: 'random' }, { key: 'sequential', text: 'Sequential', value: 'sequential' }]} />
                    <Form.Input label='Total experimental trials' type='number' fluid value={this.state.params.nbTrials} onChange={(event, data) => this.setState({
                    params: {
                      ...this.state.params,
                    ...
                    <Form.Input label='Total practice trials' type='number' fluid value={this.state.params.nbPracticeTrials} onChange={(event, data) => this.setState({
                    params: {
                      ...this.state.params,
            ...
            <Table basic='very'>
              ...
              <Table.Body className={styles.trialsTable}>
                {this.state.params.stimuli && this.state.params.stimuli.map((e, num) => <StimuliRow key={num} num={num} conditions={[1, 2, 3, 4].map(n => this.state.params[`stimulus${n}`].title)} {...e} onDelete={num => {
        ...
        return <Grid>
            <Grid.Column width={8} style={{ display: 'grid', alignContent: 'space-between' }}>
              ...
              <Segment basic style={{ marginTop: '100px' }}>
                <ParamSlider label='ITI Duration (seconds)' value={this.state.params.iti} marks={{
                1: '0.25',
                2: '0.5',
                3: '0.75',
                4: '1',
                ...
                7: '1.75',
                8: '2'
              }} ms_conversion='250' onChange={value => this.setState({
            ...
            <Grid.Column width={8} style={{ display: 'grid', alignContent: 'space-between' }}>
              ...
              <Segment basic>
                <Checkbox defaultChecked={this.state.params.selfPaced} label='Self-paced data collection' onChange={value => this.setState({
                params: { ...this.state.params, selfPaced: !this.state.params.selfPaced },
                saved: false
              })} />
              </Segment>

              {!this.state.params.selfPaced ? <Segment basic>
                  <ParamSlider label='Presentation time (seconds)' value={this.state.params.presentationTime} marks={{
                1: '0.25',
                2: '0.5',
                3: '0.75',
                4: '1',
        ...
        return <Grid stretched>
            <Grid.Column width={8} stretched style={{ display: 'grid', alignContent: 'space-between' }}>
              <Segment basic>
                ...
                <Form>
                  <Form.TextArea autoHeight value={this.state.params.intro} placeholder="e.g., You will view a series of faces and houses. Press 1 when a face appears and 9 for a house. Press the the space bar on your keyboard to start doing the practice trials. If you want to skip the practice trials and go directly to the task, press the 'q' button on your keyboard." onChange={(event, data) => this.setState({
            ...
            <Grid.Column width={8} stretched style={{ display: 'grid', alignContent: 'space-between' }}>
              <Segment basic>
                ...
                <Form>
                  <Form.TextArea autoHeight value={this.state.params.taskHelp} placeholder='e.g., Press 1 for a face and 9 for a house' onChange={(event, data) => this.setState({
      ...
      case CUSTOM_STEPS.PREVIEW:
        return <Grid relaxed padded className={styles.contentGrid}>
            <Grid.Column stretched width={14} textAlign='right' verticalAlign='middle' className={styles.previewWindow}>
              <PreviewExperimentComponent {...loadProtocol(this.props.paradigm)} isPreviewing={this.state.isPreviewing} onEnd={this.endPreview} type={this.props.type} paradigm={this.props.paradigm} previewParams={this.props.params} title={this.props.title} />
            </Grid.Column>
