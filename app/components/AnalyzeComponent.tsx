...
import { Grid, Icon, Segment, Header, Dropdown, Divider, Button, Checkbox, Sidebar } from "semantic-ui-react";
import { isNil } from "lodash";
import Plot from "react-plotly.js";
import styles from "./styles/common.css";
import { DEVICES, MUSE_CHANNELS, EMOTIV_CHANNELS, KERNEL_STATUS, EXPERIMENTS } from "../constants/constants";
import { readWorkspaceCleanedEEGData, getSubjectNamesFromFiles, readWorkspaceBehaviorData, readBehaviorData, storeAggregatedBehaviorData } from "../utils/filesystem/storage";
...
export default class Analyze extends Component<Props, State> {
  ...
  constructor(props: Props) {
    super(props);
    this.state = {
      activeStep: this.props.isEEGEnabled === true ? ANALYZE_STEPS.OVERVIEW : ANALYZE_STEPS.BEHAVIOR,
  ...
  handleBehaviorDatasetChange(event: Object, data: Object) {
    const {
      dataToPlot,
      layout
    } = aggregateDataForPlot(readBehaviorData(data.value), this.state.selectedDependentVariable, this.state.removeOutliers, this.state.showDataPoints, this.state.displayMode);
    this.setState({
  ...
  handleDependentVariableChange(event: Object, data: Object) {
    const {
      dataToPlot,
      layout
    } = aggregateDataForPlot(readBehaviorData(this.state.selectedBehaviorFilePaths), data.value, this.state.removeOutliers, this.state.showDataPoints, this.state.displayMode);
    this.setState({
  ...
  handleRemoveOutliers(event: Object, data: Object) {
    const {
      dataToPlot,
      layout
    } = aggregateDataForPlot(readBehaviorData(this.state.selectedBehaviorFilePaths), this.state.selectedDependentVariable, !this.state.removeOutliers, this.state.showDataPoints, this.state.displayMode);
    this.setState({
  ...
  handleDisplayModeChange(displayMode) {
    if (this.state.selectedBehaviorFilePaths && this.state.selectedBehaviorFilePaths.length > 0) {
      const {
        dataToPlot,
        layout
      } = aggregateDataForPlot(readBehaviorData(this.state.selectedBehaviorFilePaths), this.state.selectedDependentVariable, this.state.removeOutliers, this.state.showDataPoints, displayMode);
      this.setState({
        dataToPlot,
        layout,
        displayMode,
  ...
  renderHelpContent() {
    switch (this.state.helpMode) {
      case 'datapoints':
        return this.renderHelp('Data Points', `In this graph, each dot refers to one data point, clustered by group (e.g., conditions).
      ...
      case 'errorbars':
        return this.renderHelp('Bar Graph', `Bar graphs are the most common way to summarize data.
      ...
      case 'whiskers':
        return this.renderHelp('Box Plot', `Box plots summarize the data in a more informative way:
      ...
      case 'outliers':default:
        return this.renderHelp('Outliers', `A datapoint is tagged as an “outlier” if its value exceeds 2 standard deviations below or above the mean of all data in the group.
  ...
  renderHelp(header: string, content: string) {
    return <>
        <Segment basic className={styles.helpContent}>
          <Button circular size='large' floated='right' icon='x' className={styles.closeButton} onClick={this.toggleDisplayInfoVisibility} />
  ...
  renderSectionContent() {
    switch (this.state.activeStep) {
      case ANALYZE_STEPS.OVERVIEW:default:
        return <>
            <Grid.Column width={4}>
              <Segment basic textAlign='left' className={styles.infoSegment}>
                ...
                <Dropdown fluid multiple selection closeOnChange value={this.state.selectedFilePaths} options={this.state.eegFilePaths} onChange={this.handleDatasetChange} />
                {this.renderEpochLabels()}
              </Segment>
            </Grid.Column>
            <Grid.Column width={8}>
              <JupyterPlotWidget title={this.props.title} imageTitle={`${this.concatSubjectNames(this.state.selectedSubjects)}-Topoplot`} plotMIMEBundle={this.props.topoPlot} />
            </Grid.Column>
          </>;
        ...
        return <>
            <Grid.Column width={4} className={styles.analyzeColumn}>
              <Segment basic textAlign='left' className={styles.infoSegment}>
                ...
                <ClickableHeadDiagramSVG channelinfo={this.props.channelInfo} onChannelClick={this.handleChannelSelect} />
                <Divider hidden />
                {this.renderEpochLabels()}
              </Segment>
            </Grid.Column>
            <Grid.Column width={8}>
              <JupyterPlotWidget title={this.props.title} imageTitle={`${this.concatSubjectNames(this.state.selectedSubjects)}-${this.state.selectedChannel}-ERP`} plotMIMEBundle={this.props.erpPlot} />
            </Grid.Column>
          </>;
        ...
        return <>
            <Grid.Column width={4}>
              <Segment basic textAlign='left' className={styles.infoSegment}>
                ...
                <Dropdown fluid multiple selection search closeOnChange value={this.state.selectedBehaviorFilePaths} options={this.state.behaviorFilePaths} onChange={this.handleBehaviorDatasetChange} onClick={this.handleDropdownClick} />
                <p />
                <Divider hidden />
                <span className='ui header'>Dependent Variable</span>
                <p />
                <Dropdown fluid selection closeOnChange value={this.state.selectedDependentVariable} options={this.state.dependentVariables} onChange={this.handleDependentVariableChange} />
              </Segment>
            </Grid.Column>
            ...
            justifyContent: 'center'
              ...
              <Segment basic textAlign='left' className={styles.plotSegment}>
                ...
                <p />
                <Checkbox checked={this.state.removeOutliers} label='Remove Response Time Outliers' onChange={this.handleRemoveOutliers} />

                <p />
                <Button.Group>
                  <Button className='tertiary' toggle active={this.state.displayMode === 'datapoints'} onClick={() => this.handleDisplayModeChange('datapoints')}>
                    Data Points
                  </Button>
                  <Button className='tertiary' toggle active={this.state.displayMode === 'errorbars'} onClick={() => this.handleDisplayModeChange('errorbars')}>
                    Bar Graph
                  </Button>
                  <Button className='tertiary' toggle active={this.state.displayMode === 'whiskers'} onClick={() => this.handleDisplayModeChange('whiskers')}>
                    Box Plot
                  </Button>
                </Button.Group>
  ...
  render() {
    return <div className={styles.mainContainer}>
        <SecondaryNavComponent title='Analyze' steps={this.props.isEEGEnabled === true ? ANALYZE_STEPS : ANALYZE_STEPS_BEHAVIOR} activeStep={this.state.activeStep} onStepClick={this.handleStepClick} />
