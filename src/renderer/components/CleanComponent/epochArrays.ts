...
// Downsample to `width` columns, preserving spikes by keeping the min AND max
// within each column (so a sharp transient is never averaged away). Returns
// [min, max] per column. Callers only downsample, i.e. width < series length.
