...
interface Props {
  ...
  /** Initial field value; defaults to the next free name. */
  defaultValue?: string;
...
export default function WorkspaceNameDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onCancel()}>
      ...
      >
        {/* Mounted per open so the field resets to a fresh suggestion. */}
        <NameForm onCancel={onCancel} {...rest} />
      </DialogContent>
    </Dialog>
  );
}
...
function NameForm({
  ...
  const [value, setValue] = useState(defaultValue ?? suggestion.name);
    ...
    >
      <div className="flex flex-col gap-[8px] pr-[32px]">
        ...
        <DialogDescription className={HOME_DIALOG_CLASSES.description}>
          You’re starting from the {templateName} template. Your recordings and
          results will be saved under this name.
        </DialogDescription>
      </div>
